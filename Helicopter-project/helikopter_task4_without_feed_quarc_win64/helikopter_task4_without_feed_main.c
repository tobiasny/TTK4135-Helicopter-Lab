...
typedef RT_MODEL_helikopter_task4_wit_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * helikopter_task4_without_feed(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helikopter_task4_without_fee_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helikopter_task4_without_fee_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://helikopter_task4_without_feed:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helikopter_task4_without_feed -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = helikopter_task4_without_feed();
  if (rtmGetErrorStatus(S) != NULL) {
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (helikopter_task4_without_fee_DW.HILReadEncoderTimebase_Task, (t_clock)
         helikopter_task4_without_feed_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
